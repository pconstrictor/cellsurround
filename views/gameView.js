...
JSGRID.SquareGridView = function (gameModel, appId, resetFuncString) {
	...
	// The view is almost 2W x 2H (whereas the model is just W x H, but each cell is 3x3 internally, with overlapping edges).
	...
	// The array includes both lines and cells (sparse: has gaps for vertices and empty cells).
	// Each line is stored as a simple boolean. Each owned cell is a player object.
	this.tdHtml = function (viewPm, i, j) {
		...
		if ((i+j).isOdd()) {  // i.e. exactly one is even, so it's a side
			click = ' onclick="JSGRID.' + this.appId + '.gridView.fillLine(this.id);" ';
		}
	...
	this.refresh = function () {   //TODO: split the HTML bits out into a template file?
		var h = this.gridModel.height;
		var w = this.gridModel.width;

		// Initialize the UI table, whose dimensions are bigger than the model's.
		var viewPm = [];
		...
		// But loop over the model when actually filling it in. (We'll do extra writes to viewPm, but oh well.)
		...
		t.push('Width: <input type="number" id="' + tmp + '" value="' + this.gridModel.width + '" min="2" max="99" /> ');
		tmp = this.makeElementId('height');
		t.push('Height: <input type="number" id="' + tmp + '" value="' + this.gridModel.height + '" min="2" max="99" /> ');
		tmp = this.makeElementId('players');
		t.push('Players: <input type="number" id="' + tmp + '" value="' + this.gridModel.players.all().length + '" min="2" max="9" /> ');

		t.push('<button type="button" onclick="' + resetFuncString + '">Reset</button>');  //TODO: split out into presenter?
		t.push('<br/>' + msg);
