...
//JSGRID is our one global variable, our 'namespace', so as to not pollute the global namespace.
var JSGRID = {};
...
/* this didn't work for FF debug...
JSGRID.Player.prototype.toString = function () {
	return '' + name;
}
*/
...
JSGRID.PlayerList = function (count) {
	...
	for (var i=0; i<count; i++) {
		this._players[i] = new JSGRID.Player('Player ' + (i+1), 'p' + (i+1), colors[i]);
	}
...
JSGRID.Game = function(gridModel) {
	...
	this.statusMessage = function () { 
		...
		if (totalScore === totalPossible) {
		    ...
		    '<br/>' +
		    "Current scores: " + ps.allScores();  // TODO: eliminate the <br/>
		}
		return msg;
...
/*
JSGRID.Side.prototype.toString = function () {
	return this.filled ? 'Y' : 'N';
}
*/	 
...
JSGRID.Cell = function (sides) {
	...
	// If the cell is now surrounded and wasn't before, assign it to the current player.
...
JSGRID.SquareGrid = function (width, height, players) {
	...
	this.reset = function (w, h, players) {
		...
		// fill matrix
		var toLeft = null, above = null; // these will be used for cells sharing sides
